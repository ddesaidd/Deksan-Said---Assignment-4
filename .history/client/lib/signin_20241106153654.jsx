...
//import Icon from '@mui/material/Icon'; // Updated import
...
import { Navigate, useLocation } from 'react-router-dom';
import auth from './auth-helper.js';
import { signin } from './api-auth.js'; // Import the signin function
...
export default function Signin(props) {
  ...
  const clickSubmit = async () => {
    ...
    try {
      const data = await signin(user);  // Calling the signin function

      // Check if the response is valid and contains error
      if (data && data.error) {
        setValues({ ...values, error: data.error });
      } else if (data && data.token) {
        // If signin is successful, authenticate and redirect
        auth.authenticate(data, () => {
          setValues({ ...values, error: '', redirectToReferrer: true });
        });
      } else {
        // If the response is empty or doesn't contain token
        setValues({ ...values, error: 'Something went wrong. Please try again later.' });
      }
    } catch (err) {
      // Handle any errors in the signin process
      setValues({ ...values, error: 'Error signing in. Please try again later.' });
